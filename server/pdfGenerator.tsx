...
import { Document, Page, Text, View, StyleSheet, Font, pdf, renderToBuffer } from '@react-pdf/renderer';

// Use default fonts - @react-pdf/renderer has built-in Arabic support
// No need to register external fonts that might fail

// Text normalization function for Arabic text
...
const styles = StyleSheet.create({
  page: {
    ...
    padding: 30,
    fontFamily: 'Helvetica', // Use default font that supports Arabic
    direction: 'rtl',
    textAlign: 'right'
  },
  title: {
  ...
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.arabicText}>اسم المشروع: {normalizeArabicText(data.projectName || 'غير محدد')}</Text>
        <Text style={styles.arabicText}>نوع المشروع: {normalizeArabicText(data.projectType || 'غير محدد')}</Text>
        <Text style={styles.arabicText}>مستوى التعقيد: {normalizeArabicText(data.technicalComplexity || 'غير محدد')}</Text>
      </View>
...
export async function generateArabicPDF(data: any): Promise<Buffer> {
  try {
    ...
    const pdfBuffer = await renderToBuffer(<ArabicPDFDocument data={data} />);
